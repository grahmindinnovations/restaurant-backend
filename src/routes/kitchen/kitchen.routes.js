...
import { readMenu, readOrders } from '../../utils/firestoreHelpers.js'
...
export function createKitchenRouter({ io }) {
  ...
  router.post('/kitchen/orders/:id/finish', requireAuth, requireAnyRole('kitchen', 'admin'), async (req, res) => {
    const db = getDb()
    ...
    try {
      ...
      // Table stays occupied until reception marks available or order is paid/cancelled.
      try {
